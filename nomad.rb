...
class NomadJobExecutor
  ...
  def deploy!(
    ...
    client.get_until("/v1/evaluation/#{eval_id}", {}, {}, proc { |r| p "Get evaluation #{eval_id}"; p r.keys.inspect; p r[:JobID]; p r[:Status]; r[:Status] == wait })
    ...
    allocations.each do |allocation|
      result = client.get_until(
        ...
        {},
        {},
        proc { |r| "Get allocation #{allocation[:ID]}"; p r.keys.inspect; p r[:JobID]; p r[:Status]; r[:ClientStatus] == 'complete' },
